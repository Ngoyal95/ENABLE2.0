...
def mapPtDataExcel(wsP,wsC,compRow,patient,colHeaders):
	...
	for key,exam in patient.exams.items():
		if exam.ignore is False:
			examData = pullExamData(exam)
			col = len(examData) #skip to columns where we print lesion data, +1 due to 1 indexing
			examData[0] = key
...
def pullExamData(exam):
	#function to extract exam data and place in list
	return [exam.enum,exam.date,exam.modality]
...
def pullLesionData(lesion):
	...
	lesion.subtype,lesion.series,lesion.slice,lesion.recistdia,lesion.longdia,\
